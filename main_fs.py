...
from util.scannet_v2_fs import (
    Scannetv2_FS,
    Scannetv2_FS_TEST,
)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.optimizer == "SGD":
        ...
        for module_name, module in model.named_modules():
            for module_param_name, value in module.named_parameters(recurse=False):
                ...
                train_list = ["UF_head", "classifier"]
                if "encoder" in module_name and (
                    all(key not in module_name for key in train_list)
                ):
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        sampler=val_sampler,
        collate_fn=partial(collate_fn_limit_fs, include_scene_names=args.forvis),
    )
...
def train(train_loader, model, optimizer, epoch, scaler, scheduler, train_calsses):
