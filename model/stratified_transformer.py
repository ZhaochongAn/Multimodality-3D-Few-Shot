...
def get_indice_pairs(
    ...
    index_0 = p2v_map.unsqueeze(-1).expand(-1, -1, k)[mask_mat]  # [M, ] for colum
...
class Stratified(nn.Module):
    def __init__(
        ...
        self.UF_head = copy.deepcopy(self.layers[-1])

        self.mlp_decode_layer = nn.Sequential(
            nn.Linear(channels[1] + channels[2], channels[1] + channels[2]),
            nn.BatchNorm1d(channels[1] + channels[2]),
            nn.ReLU(inplace=True),
            nn.Linear(channels[1] + channels[2], last_dim),
        )
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx, gt, query_base_y=None):
        ...
        for i, layer in enumerate(self.layers):
            if i == len(self.layers) - 1:
                UF_feats = self.UF_head(feats, xyz, offset, gt, query_base_y)[0]
        ...
        IF_feats = self.mlp_decode_layer(IF_feats)
        return (
            UF_feats_cat,
            xyz_stack[-2],
            offset_stack[-2],
