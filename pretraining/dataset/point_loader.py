...
class Point3DLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if memcache_init and (
            not exists(
                "/dev/shm/%s_%s_%06d_locs_%08d" % (dataset_name, split, identifier, 0)
            )
        ):
            ...
            for i, (locs, feats, labels) in enumerate(
                ...
                sa_create(
                    "shm://%s_%s_%06d_locs_%08d" % (dataset_name, split, identifier, i),
                    locs,
                )
                sa_create(
                    "shm://%s_%s_%06d_feats_%08d"
                    % (dataset_name, split, identifier, i),
                    feats,
                )
                sa_create(
...
class Point3DLoader_strat_withmax_points(torch.utils.data.Dataset):
    """Dataloader for 3D points and labels."""
    ...
    def __getitem__(self, index_long):
        ...
        locs, feats, labels, inds = feat_point_voxelize(
            locs,
            feats_in,
            labels_in,
