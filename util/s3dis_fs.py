...
class S3DIS_FS(S3DIS_base):
    ...
    def __init__(
        ...
        if split == "train":
            ...
            raise NotImplementedError("Unkown mode %s! [Options: train/test]" % split)
    ...
    def sample_pointcloud(self, scan_name, sampled_classes, sampled_class, support):
        ...
        if support:
            ...
            for i, value in enumerate(label):
                ...
                elif value.item() in self.test_classes and self.split == "train":
                    test_label[i] = 255
...
class S3DIS_FSForVIS(S3DIS_FS):
    ...
    def crop_point_cloud(self, point_cloud, feat, labels):
        ...
        q1_count = np.sum((point_cloud[:, 0] <= half_x) & (labels == self.target_cls))
        q2_count = np.sum((point_cloud[:, 0] > half_x) & (labels == self.target_cls))
        q3_count = np.sum((point_cloud[:, 1] > half_y) & (labels == self.target_cls))
        q4_count = np.sum((point_cloud[:, 1] <= half_y) & (labels == self.target_cls))
